...
class NetworkanalyzerState extends State<Networkanalyzer> {
  ...
  String serverAddress = 'https://kali.tail7d5586.ts.net/nmap.php';
  final List<String> targetSpecOptions = ['-iL', '-iR', '--exclude','--excludefile'];
  final List<String> hostDiscoveryOptions = [ '-sL', '-sn', '-Pn', '-PS', '-PA', '-PU', '-PY', '-PE', '-PP', '-PM',
    '-PO', '-n', '-R', '--dns-servers', '--system-dns', '--traceroute'];
  final List<String> scanTechOptions = [ '-sS', '-sT', '-sA', '-sW', '-sM', '-sU', '-sN', '-sF', '-sX',
    '--scanflags', '-sI', '-sY', '-sZ', '-sO', '-b'];
  final List<String> portSpecOptions = [ '-p', '--exclude-ports', '-F', '-r', '--top-ports', '--port-ratio','--open'];
  final List<String> serviceVersionOptions = [ '-sV','--version-intensity', '--version-light', '--version-all',
    '--version-trace'];
  final List<String> osDetectionOptions = ['-O', '--osscan-limit', '--osscan-guess'];
  final List<String> timingPerformanceOptions = ['T0','-T1', '-T2','-T3', '-T4','-T5','--min-hostgroup', '--max-hostgroup', '--min-parallelism',
  ...
  final List<String> firewallIDSOptions = ['-f', '--mtu', '-D', '-S', '-e', '-g', '--source-port', '--proxies',
    '--data', '--data-string', '--data-length', '--ip-options', '--ttl',
    '--spoof-mac', '--badsum'];
  final List<String> outputOptions = ['-oN','-oX','-oG'];
  final List<String> miscOptions = [ '-6', '-A', '--datadir', '--send-eth', '--send-ip', '--privileged',
    '--unprivileged', '-V', '-h'];
  final List<String> scriptOptions = ['-sC','--script','--script-args','--script-args-file','--script-trace','--script-updatedb','--script-help'];
  ...
  void updateCommand() {
    ...
    if (selectedTimingPerformance != null) command += ' $selectedTimingPerformance';
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        children: [
          ...
          buildDropdownRow('Script', scriptOptions, 'Script List', scriptlistOptions,
                  (value) {
                setState(() {
                  selectedOutput = value;
                  updateCommand();
